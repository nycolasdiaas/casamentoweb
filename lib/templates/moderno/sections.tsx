...
function NumberedHead({ n, label }: { n: string; label: string }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      <span
        className="font-[family-name:var(--font-script)] text-[11px]"
        style={{ color: "var(--accent)" }}
      >
        {n}
      </span>
      <div className="flex-1 h-px" style={{ background: "var(--ink)" }} />
      <span className="font-[family-name:var(--font-script)] text-[10.5px] tracking-[0.2em] uppercase">
        {label}
      </span>
    </div>
  );
...
function Caption({ left, right }: { left: string; right?: string | null }) {
  return (
    <div
      className="flex justify-between gap-3 px-5 pt-2.5 font-[family-name:var(--font-script)] text-[9.5px] tracking-[0.18em] uppercase"
  ...
  return (
    <section className="pt-8">
      <div className="flex items-start justify-between gap-2 px-5">
        <h1 className="font-[family-name:var(--font-display)] font-black text-[clamp(44px,15vw,72px)] leading-[0.88] tracking-[-0.04em] uppercase">
      ...
      <div className="flex justify-between items-baseline gap-3 px-5 pt-5 pb-4">
        ...
        <div className="font-[family-name:var(--font-script)] text-[10.5px] tracking-[0.12em] uppercase text-right leading-relaxed">
      ...
      <div
        className="flex justify-between px-5 py-3 font-[family-name:var(--font-script)] text-[10px] tracking-[0.16em] uppercase"
...
export async function Story({ content, siteId }: SectionProps) {
  ...
  return (
    <section className="pt-2 pb-14">
      ...
      <h2 className="mx-5 font-[family-name:var(--font-display)] font-black text-[38px] leading-[0.95] tracking-[-0.03em] uppercase">
        Nossa história
        <span style={{ color: "var(--accent)" }}>.</span>
      </h2>

      <p
        className="mx-5 mt-5 text-[15px] leading-relaxed whitespace-pre-line"
        ...
        <div className="mt-6 grid grid-cols-2 gap-3.5 px-5 items-start">
          {duas.map((foto, i) => (
            ...
            <figure key={foto.id} className={`mb-0 ${i === 0 ? "mt-8" : "m-0"}`}>
              ...
              <figcaption
                className="mt-2 font-[family-name:var(--font-script)] text-[9.5px] tracking-[0.18em] uppercase"
                style={{ color: tinta(60) }}
              >
  ...
  return (
    <section className="px-5 pt-2 pb-14">
      ...
      {itens.map((item, i) => (
        ...
        >
          <div
            className="w-16 shrink-0 font-[family-name:var(--font-script)] text-[11px] tracking-[0.16em] uppercase pt-1"
            style={{ color: "var(--accent)" }}
          >
            {item.time}
          </div>
          <div className="flex-1">
            <div
              className="font-[family-name:var(--font-script)] text-[9.5px] tracking-[0.26em] uppercase"
              style={{ color: tinta(60) }}
            >
              {item.label}
            </div>
            <div className="mt-1.5 font-[family-name:var(--font-display)] font-bold text-[20px] leading-snug tracking-[-0.01em]">
              {item.title}
            </div>
            {item.text && (
              <p className="mt-1.5 text-[14px] leading-relaxed" style={{ color: tinta(80) }}>
                {item.text}
              </p>
            )}
            {item.href && item.cta && (
              <a
                ...
                className="inline-block mt-2.5 font-[family-name:var(--font-script)] text-[10px] tracking-[0.2em] uppercase"
                style={{ color: "var(--accent)" }}
              >
                {item.cta}
              </a>
...
export function Rsvp({ slug }: SectionProps) {
  return (
    <section className="px-5 pt-2 pb-14">
      ...
      <h2 className="font-[family-name:var(--font-display)] font-black text-[38px] leading-[0.95] tracking-[-0.03em] uppercase">
        Você vem
        <span style={{ color: "var(--accent)" }}>?</span>
      </h2>

      <div className="mt-5 pl-4" style={{ borderLeft: `3px solid var(--accent)` }}>
        <p className="text-[15px] leading-relaxed" style={{ color: tinta(85) }}>
      ...
      <Link
        href={`/s/${slug}`}
        className="inline-block mt-6 font-[family-name:var(--font-script)] text-[10.5px] tracking-[0.22em] uppercase px-7 py-4 transition-opacity hover:opacity-85"
...
export async function Gifts({ siteId, content }: SectionProps) {
  ...
  return (
    ...
    >
      ...
      <p className="mb-6 text-[15px] leading-relaxed" style={{ color: tinta(85) }}>
        {content.giftMessage ??
  ...
  return (
    <section className="px-5 pt-2 pb-14">
      ...
      <div className="pl-4" style={{ borderLeft: `3px solid var(--accent)` }}>
        <div className="font-[family-name:var(--font-display)] font-black text-[30px] leading-[0.98] tracking-[-0.03em] uppercase">
          As fotos
          <br />
          vêm depois
          <span style={{ color: "var(--accent)" }}>.</span>
        </div>
        <p className="mt-3 text-[14.5px] leading-relaxed" style={{ color: tinta(80) }}>
...
export function Footer({ content }: SectionProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="mt-6 pt-4 flex justify-between gap-3 font-[family-name:var(--font-script)] text-[10px] tracking-[0.18em] uppercase"
        style={{

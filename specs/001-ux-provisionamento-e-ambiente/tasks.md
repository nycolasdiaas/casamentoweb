...
- [ ] **T001** Escrever `lib/baseUrl.test.ts` cobrindo a cascata de descoberta do endereço,
...
- [ ] **T002** Reescrever `lib/baseUrl.ts`: `getBaseUrl()` e `baseUrlEstatica()` passam a
...
- [ ] **T003** Rodar `npm run test lib/baseUrl.test.ts` e confirmar os 7 casos verdes. —
...
- [ ] **T004** `lib/site/provision.ts`: tornar `baseUrl` opcional de verdade — sem
  endereço, o site nasce igual e `previewUrl` fica nulo. — `[UX-001][FR-011]`

- [ ] **T005** `lib/site/provision.ts`: copiar para `site_content`, no INSERT do site novo,
...
- [ ] **T006** [P] Teste de `provisionSiteForOrder` cobrindo T004 e T005: sem `baseUrl` o
...
- [ ] **T007** `app/actions/account-actions.ts`: tirar `await getBaseUrl()` de dentro do
...
- [ ] **T011** [P] `app/actions/invite-actions.ts`: tolerar ausência de endereço ao criar e
  ao publicar convite — o convite é criado e o endereço aparece quando houver. —
  `[UX-002][FR-003][FR-005]`

- [ ] **T012** [P] `app/conta/convites/[conviteId]/page.tsx` e
...
- [ ] **T013** [P] `app/error.tsx` e `app/global-error.tsx`: tela de erro em português, na
  voz da Enlace, com "tentar de novo" e caminho de volta. Texto revisado pela Skill
  `texto-do-casal`. — `[UX-002][FR-008]`

- [ ] **T014** **Verificação no navegador (E2E)**: em produção, abrir a aba Convites e
...
- [ ] **T015** [P] `app/conta/pedidos/[id]/convidados/page.tsx`: mostrar o endereço da
  família como texto ao lado do botão de copiar, para o casal conferir e digitar se
  precisar. — `[UX-016][FR-013]`

- [ ] **T016** **Verificação no navegador (E2E)**: pedir o QR de um site publicado em
...
- [ ] **T017** **Verificação no navegador (E2E)**: ler as etiquetas de compartilhamento de
...
- [ ] **T018** **Verificação no navegador (E2E)**: na aba Convidados em produção, a coluna
...
- [ ] **T019** **Verificação no navegador (E2E)**: provocar uma falha de servidor numa rota
...
- [ ] **T020** `npm run lint` limpo. — `[constitution · portão 1]`
- [ ] **T021** `npx tsc --noEmit` limpo. — `[constitution · portão 2]`
- [ ] **T022** `npm run build` passando (o `next build` é a verdade). — `[constitution · portão 3]`
- [ ] **T023** `npm run test` verde, suíte inteira, uma de cada vez. — `[constitution · portão 4]`
- [ ] **T024** Atualizar o **Status** de UX-001, UX-002, UX-003, UX-005, UX-016 e UX-021 na
